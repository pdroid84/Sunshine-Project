...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    //Instead of using enum of Java used support annotation IntDef to create integer constant values
...
//    }
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        ...
        try {
            ...
            final String FORECAST_BASE_URL =
              ...
              // following base url generates "invalid result" - for testing purpose
              //      "http://google.com/?";
              //following base url generates "empty result" - for testing purpose
            ...
            if (Utility.isLocationLatLonAvailable(context)) {
                ...
                uriBuilder.appendQueryParameter(QUERY_PARAM, location);
            }

            Uri builtUri = uriBuilder.appendQueryParameter(FORMAT_PARAM, format)
                    .appendQueryParameter(UNITS_PARAM, units)
                    .appendQueryParameter(DAYS_PARAM, Integer.toString(numDays))
                    .appendQueryParameter(API_PARAM, BuildConfig.OPEN_WEATHER_API_KEY)
                    .build();
            ...
            if (buffer.length() == 0) {
                ...
                storeLocationStatus(getContext(), LOCATION_STATUS_SERVER_DOWN);
                return;
            }
            ...
            e.printStackTrace();
            storeLocationStatus(getContext(), LOCATION_STATUS_SERVER_INVALID);
        } finally {
    ...
    long addLocation(String locationSetting, String cityName, double lat, double lon) {
        ...
        String locationQuery = WeatherContract.LocationEntry.COLUMN_LOCATION_SETTING + " = ?";
        String[] locationArg = {locationSetting};
        String[] locationProj = {WeatherContract.LocationEntry._ID};
        Cursor cursor = getContext().getContentResolver().query(WeatherContract.LocationEntry.CONTENT_URI,
                locationProj, locationQuery, locationArg, null);
        long id;
        if (cursor.moveToFirst()) {
            int cIndex = cursor.getColumnIndex(WeatherContract.LocationEntry._ID);
            id = cursor.getLong(cIndex);
        } else {
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            if(forecastJson.has(OWM_MESSAGE_CODE)) {
                ...
                switch (errorCode) {
                    ...
                    case HttpURLConnection.HTTP_NOT_FOUND:
                        storeLocationStatus(getContext(), LOCATION_STATUS_INVALID);
                        return;
                    default:
                        storeLocationStatus(getContext(), LOCATION_STATUS_SERVER_DOWN);
                        return;
            ...
            if (cVVector.size() > 0) {
                ...
                insertCount = getContext().getContentResolver().bulkInsert(WeatherContract.WeatherEntry.CONTENT_URI, contentValues);
                boolean notificationNeeded = Utility.getNotificationFlag(getContext());
            ...
            //Now delete all old data to keep the database tidy and clean
            //Get's a calendar object with the current time.
            Calendar calendar = Calendar.getInstance();
            Log.v(LOG_TAG,"Current Date = " +Long.toString(WeatherContract.normalizeDate(calendar.getTimeInMillis())));
            //Add -1 to signify yesterday's date
            calendar.add(Calendar.DATE, -1);
            String yesterdayDate = Long.toString(WeatherContract.normalizeDate(calendar.getTimeInMillis()));
            Log.v(LOG_TAG, "Yesterday date = " + yesterdayDate);
            // delete old data so we don't build up an endless history
            int delCnt = getContext().getContentResolver().delete(WeatherContract.WeatherEntry.CONTENT_URI,
                    WeatherContract.WeatherEntry.COLUMN_DATE + " <= ?",
                    new String[] {yesterdayDate});
            Log.v(LOG_TAG, "Number of records deleted = " + delCnt);
        } catch (JSONException e) {
            Log.e(LOG_TAG, e.getMessage(), e);
            e.printStackTrace();
    ...
    /**
     * Method to store the location status into shared preference. This function should not be called from the UI thread
     * because this function uses the "commit" to write to shared preference
     * @param ctx Context to get the PreferenceManager from
     * @param locationStatus The IntDef value to set
     */
    public static void storeLocationStatus (Context ctx, @LocationStatus int locationStatus) {
